...
export class actionRepairExtension {
    constructor() {
    }
    ...
    IsNecessary(creep: Creep) {
        var targets = creep.room.find(FIND_STRUCTURES, {
            filter: (structure) => {
                return structure.structureType == STRUCTURE_EXTENSION && structure.hits < structure.hitsMax;
            }
        });
    ...
    Execute(creep: Creep) {
        ...
        if (structuresToRepair.length) {
            creep.say(Actions.RepairExtension);
